...
export class PrettyStacktraceProcessor extends DisplayProcessor {
  ...
  public displaySummaryErrorMessages(spec: CustomReporterResult, log: string): string {
    return this.configuration.summary.displayStacktrace === StacktraceOption.PRETTY
  ...
  private prettifyStack(stack: string) {
    ...
    filteredStack.split('\n').forEach((stackLine) => {
      if (stackRegExp.test(stackLine)) {
        ...
        const errorContext = this.retrieveErrorContext(filename, parseInt(lineNumber, 10), parseInt(columnNumber, 10))

        logs.push(
          `${this.theme.prettyStacktraceFilename(filename)}:${this.theme.prettyStacktraceLineNumber(
            lineNumber
          )}:${this.theme.prettyStacktraceColumnNumber(columnNumber)}`
        )
  ...
  private retrieveErrorContext(filename: string, lineNb: number, columnNb: number) {
    ...
    let fileLines
    try {
      fileLines = fs.readFileSync(filename, 'utf-8').split('\n')
    } catch (error) {
