...
export class SummaryDisplay {
  ...
  public display(metrics: ExecutionMetrics) {
    ...
    const execution = `Executed ${metrics.executedSpecs} of ${
      metrics.totalSpecsDefined
    }${pluralizedSpec}`;
    let status = "";
    if (metrics.failedSpecs === 0 && metrics.globalErrors.length === 0) {
      status =
        metrics.totalSpecsDefined === metrics.executedSpecs
          ? " SUCCESS".successful
          : " INCOMPLETE".pending;
    }
    const failed =
      metrics.failedSpecs > 0 ? ` (${metrics.failedSpecs} FAILED)` : "";
    const pending =
      metrics.pendingSpecs > 0 ? ` (${metrics.pendingSpecs} PENDING)` : "";
    const skipped =
      metrics.skippedSpecs > 0 ? ` (${metrics.skippedSpecs} SKIPPED)` : "";
    let errors =
      metrics.globalErrors.length > 1
        ? ` (${metrics.globalErrors.length} ERRORS)`
        : "";
    errors =
      metrics.globalErrors.length === 1
        ? ` (${metrics.globalErrors.length} ERROR)`
        : errors;
    const duration = this.configuration.summary.displayDuration
      ? ` in ${metrics.duration}`
      : "";
    ...
    this.logger.newLine();
    if (
      this.configuration.summary.displaySuccessful &&
      metrics.successfulSpecs > 0
    ) {
    ...
    if (
      this.configuration.summary.displayFailed &&
      metrics.globalErrors.length > 0
    ) {
