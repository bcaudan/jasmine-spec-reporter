...
export class SummaryDisplay {
  ...
  public display(metrics: ExecutionMetrics) {
    ...
    if (metrics.failedSpecs === 0 && metrics.globalErrors.length === 0) {
      status =
        metrics.totalSpecsDefined === metrics.executedSpecs
          ? this.theme.successful(' SUCCESS')
          : this.theme.pending(' INCOMPLETE')
    }
    ...
    this.logger.log(
      execution +
        status +
        this.theme.failed(errors) +
        this.theme.failed(failed) +
        this.theme.pending(pending) +
        this.theme.pending(skipped) +
        duration +
        '.'
    )
  ...
  private failedSummary(spec: CustomReporterResult, index: number): void {
    ...
    if (this.configuration.summary.displayErrorMessages) {
      ...
      this.logger.process(spec, (displayProcessor: DisplayProcessor, object: CustomReporterResult, log: string) => {
  ...
  private errorSummary(error: CustomReporterResult, index: number): void {
    ...
    this.logger.increaseIndent()
    this.logger.process(error, (displayProcessor: DisplayProcessor, object: CustomReporterResult, log: string) => {
      return displayProcessor.displaySummaryErrorMessages(object, log)
    })
    this.logger.decreaseIndent()
  }
